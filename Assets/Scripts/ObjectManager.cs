...
public sealed class ObjectManager : MonoBehaviour {
	...
	public readonly int SCORE_DEBRIS = 10;				// 点数	隕石
	public readonly int SCORE_PLAYER = 1000;				//		相手側プレイヤー
	public readonly float[] SCORE_MULTIPILER_DISP = new float[8] { 1, 100, 30, 10, 1, 0.5f, 0.25f, 0.25f };  // 自分サイドのプレイヤー数に応じた得点倍率
																													// 少なければ少ないほど高い
	public readonly int CNT_TIMER_SHUFFLE = 60;			// 相手サイドのプレイヤーが0になってから強制組分けが発生するまでの時間
	public readonly int CNT_CHANGE_SIDE = 3;				// 自力でサイドを変更できる回数

	public readonly int CNT_STEPS_NORMAL = 8;			// プレイヤーの1intの移動距離(通常時)
	public readonly int CNT_STEPS_TURBO = 20;			//							(ターボ時)
